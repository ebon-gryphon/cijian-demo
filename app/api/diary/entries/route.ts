...
export async function POST(request: Request) {
  try {
    ...
    try {
      ...
      if (e instanceof BetaError) throw e;
      throw new BetaError(e instanceof Error ? e.message : '日记格式不正确');
    }
    const db = getD1();
      ...
      for (const src of [p.src, p.original].filter(Boolean) as string[]) {
        if (src.startsWith('/api/diary/assets/')) {
          const found = await db
            ...
            .first();
          if (!found) throw new BetaError('图片不属于这本日记', 403);
        }
      }
    ...
    if (existing) {
      ...
      throw new BetaError('新日记请先只填写自己的片段，对方加入后可以补写');
