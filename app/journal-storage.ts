...
// Read legacy image blobs without deleting or changing the original database.
export async function restoreLegacyPictures(
  entries: Entry[],
  raw: string | null,
): Promise<Entry[]> {
  ...
  if (!Array.isArray(old.memories)) return entries;
  const images: Record<string, string> = {
    'demo-photo-noodles': '/memories/rainy-noodles.jpg',
    'demo-photo-lake': '/memories/west-lake.jpg',
    'demo-photo-breakfast': '/memories/weekend-breakfast.jpg',
  };
  const db = await new Promise<IDBDatabase>((resolve, reject) => {
    const r = indexedDB.open('between-us-attachments-v1', 1);
    r.onsuccess = () => resolve(r.result);
    r.onerror = () => reject(r.error);
  });
  try {
    for (const entry of entries) {
      const previous = old.memories.find((m) => `shared-${m.id}` === entry.id);
      for (const attachment of (previous?.attachments || [])
        .filter((a) => a.kind === 'image')
        .slice(0, 6)) {
        let src = images[attachment.id];
        if (!src && db.objectStoreNames.contains('files')) {
          const blob = await new Promise<Blob | undefined>(
            (resolve, reject) => {
              const r = db
        ...
        if (src)
          entry.pictures.push({
            id: attachment.id,
            src,
            kind: images[attachment.id] ? 'generated' : 'uploaded',
            prompt: '',
          });
      }
    }
